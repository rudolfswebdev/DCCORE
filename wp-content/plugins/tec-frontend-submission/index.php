...
function xt_tec_frontend_form_register() {
	...
	//     'text'      => array(
	//         'no_terms_text' => __( 'Sorry, no event categories could be found.', 'xtensionpress' ) 
	//     ),
	// ) );
	...
	$cmb->add_field( array(
		...
		'options' => xt_tec_frontend_get_categories( array( 'taxonomy' => 'tribe_events_cat', 'allow_empty' => false ) ),
	) );
	...
	$cmb->add_field( array(
		...
		'options' => xt_tec_frontend_get_posts( array( 'post_type' => 'tribe_venue', 'numberposts' => '-1', 'allow_empty' => true ) ),
	) );
	...
	$cmb->add_field( array(
		...
		'type'       => 'multicheck',
		'show_option_none' => false,
		'options' => xt_tec_frontend_get_posts( array( 'post_type' => 'tribe_organizer', 'numberposts' => '-1', 'allow_empty' => false ) ),
	) );
...
function xt_tec_frontend_get_categories( $query_args ) {
    $args = wp_parse_args( $query_args, array(
		'type' => 'post',
		'child_of' => 0,
		'parent' => '',
	...
	xt_tec_frontend_get_category_childs( 'slug', 0, 0, $categories, 0, $categories_dropdown );

    return $categories_dropdown;
}

function xt_tec_frontend_get_category_childs( $param = 'slug', $parent_id, $pos, $array, $level, &$dropdown ) {
	for ( $i = $pos; $i < count( $array ); $i ++ ) {
		if ( $array[ $i ]->category_parent == $parent_id ) {
			if ( $param == 'slug' ) {
				$data = array(
						$array[ $i ]->slug => str_repeat( "- ", $level ) . $array[ $i ]->name,
				);
			}
			else {
				$data = array(
						$array[ $i ]->term_id => str_repeat( "- ", $level ) . $array[ $i ]->name,
				);
			}
			$dropdown = array_merge( $dropdown, $data );
			xt_tec_frontend_get_category_childs( $param, $array[ $i ]->term_id, $i, $array, $level + 1, $dropdown );
		}
	}
}
...
function xt_tec_frontend_submission_handle( $cmb, $post_data = array() ) {
	...
	if ( isset( $sanitized_values['event_starttime'] ) && isset( $sanitized_values['event_endtime'] ) && trim( $sanitized_values['event_starttime'] ) && trim( $sanitized_values['event_endtime'] ) ) {
		...
		if ( function_exists('tribe_beginning_of_day') ) {
			$sanitized_values['event_startdate'] = tribe_beginning_of_day( $sanitized_values['event_startdate'] );
		}
		elseif ( function_exists('tribe_event_beginning_of_day') ) {
		...
		if ( function_exists('tribe_end_of_day') ) {
			$sanitized_values['event_enddate'] = tribe_end_of_day( $sanitized_values['event_enddate'] );
		}
		elseif ( function_exists('tribe_event_end_of_day') ) {
	...
	if ( isset( $sanitized_values['event_category'] ) ) {
		if ( !empty($sanitized_values['event_category']) && is_array($sanitized_values['event_category']) ) {
			$event_category = array_values($sanitized_values['event_category']);
		}
		wp_set_object_terms( $new_submission_id, $event_category, 'tribe_events_cat' );
	}
	...
	if ( isset( $sanitized_values['event_organizer'] ) ) {
		if ( !empty($sanitized_values['event_organizer']) && is_array($sanitized_values['event_organizer']) ) {
			foreach ($sanitized_values['event_organizer'] as $event_organizer) {
				add_post_meta( $new_submission_id, '_EventOrganizerID', $event_organizer, false );
			}
		}
	}
	...
	global $xt_tect_frontend_post_id;
	$xt_tect_frontend_post_id = $new_submission_id;
