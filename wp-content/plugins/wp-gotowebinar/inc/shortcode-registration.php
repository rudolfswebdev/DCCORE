...
function go_to_webinar_registration( $atts ) {
    ...
    if($a['key'] == "upcoming") { 
        ...
        foreach ($jsondata as $data) {
            ...
            if(($a['timezone'] == 'fGnTS2Jw' || $a['timezone'] == $data['timeZone']) && ($a['include'] == 'QZRS7VSd' || strpos($data['subject'], $a['include']) !== false)  && ($a['exclude'] == 'uc4hvp8K' || strpos($data['subject'], $a['exclude']) === false)){
    ...
    if ($getTransientForm != false && !isset($options['gotowebinar_disable_cache'])){
...
$html .= '</select>';
    ...
    $html .= '<tr><td><label for="gotowebinar_opt_in">'.$customSubscribeConditionMessage.'</label></td>';
    ...
    $html .= '<tr><td><label class="second-opt-in" for="gotowebinar_opt_in">'.$customRegistrationConfirmationMessage.'</label></td>';
    $html .= '<td><input class="second-opt-in" name="gotowebinar_opt_in_second" id="gotowebinar_opt_in_second" type="checkbox" checked></td></tr>'; 
    }     
