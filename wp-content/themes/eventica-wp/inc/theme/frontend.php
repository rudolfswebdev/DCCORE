...
add_filter( 'body_class', 'tokopress_body_class_ltr' );
function tokopress_body_class_ltr( $classes ) {
	if ( !is_rtl() ) {
		$classes[] = 'ltr';
	}
	return $classes;
...
add_filter( 'wp_nav_menu_items', 'tokopress_header_menu_item_mininav', 10, 2 );
function tokopress_header_menu_item_mininav ( $items, $args ) {
	if ( function_exists('ubermenu') ) {
		return $items;
	}
	if ( $args->theme_location == 'header_menu' ) {
		$items .= '<li class="menu-item menu-item-mininav"><a href="javascript:void(0)" class="sb-toggle-left"><i class="fa fa-navicon"></i><span class="mininav">'.esc_html__('Menu', 'tokopress').'</span></a></li>';
	}
	return $items;
}

function tokopress_is_not_home_page() {
