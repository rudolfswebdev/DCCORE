...
add_action( 'add_meta_boxes', 'tokopress_metabox_event_details_add', 15 );
...
function tokopress_metabox_event_details_output( $post ) {
	...
	$ticket_options[''] = __( 'No', 'tokopress' );
	if ( class_exists( 'woocommerce' ) ) {
		$ticket_options['tokopress'] = __( 'Yes', 'tokopress' );
	}
	...
	<table class="form-table">
		<tr>
			<th>
				<label for="tokopress_event_text"><?php _e( 'Sell Ticket with Eventica & WooCommerce', 'tokopress' ); ?>:</label>
			</th>
			<td>
				...
				if ( $ticket_id ) {
					$ticket_edit_query = array(
						'action' => 'edit',
						'post' => $ticket_id
					);
					$ticket_edit_url = esc_url( add_query_arg( $ticket_edit_query, admin_url( 'post.php' ) ) );
					...
					echo '<span class="description"><a href="'.$ticket_edit_url.'" target="_blank">'.__( 'Edit in WooCommerce', 'tokopress' ).'</a> <a href="'.$ticket_report_url.'" target="_blank">'.__( 'View Sales Report', 'tokopress' ).'</a></span>';
				}
				?>
				<br>
				<span class="description">
					<?php _e( 'This option will be <b>IGNORED</b> if you have active ticket for this event using Event Tickets or Event Tickets Plus plugin.', 'tokopress' ); ?>
				</span>
			</td>
	    </tr>
		...
		<tr>
			...
			<td>
				...
				<span class="description">
					<?php _e( 'It is useful if you want to link Call To Action button to third party website.', 'tokopress' ); ?><br/>
					<?php _e( 'By default, it will use event website if available.', 'tokopress' ); ?>
				</span>
			</td>
	    </tr>
		<tr>
