 ...
 */
add_filter( 'body_class', 'tokopress_wc_product_image_layout_class' );
function tokopress_wc_product_image_layout_class( $classes ) {
	if ( !is_product() )
		return $classes;

	$layout = of_get_option( 'tokopress_wc_product_image_layout' );
	if ( !$layout ) 
		$layout = 'full';
	$classes[] = 'wc-product-image-'.$layout;

	return $classes;
}

if ( of_get_option( 'tokopress_wc_product_image_layout' ) != 'half' ) {
	/**
...
if( of_get_option( 'tokopress_minicart_header' ) ) {
	add_filter('woocommerce_add_to_cart_fragments', 'tokopress_wc_header_add_to_cart_fragment');
}
function tokopress_wc_header_add_to_cart_fragment( $fragments ) {
	$fragments['.mobile-menu a .minicart, .header-menu a .minicart'] = '<span class="minicart">'. WC()->cart->get_cart_subtotal() .'</span>';
	return $fragments;
}

add_filter( 'wp_nav_menu_items', 'tokopress_wc_header_menu_item_minicart', 10, 2 );
function tokopress_wc_header_menu_item_minicart ( $items, $args ) {
	if ( function_exists('ubermenu') ) {
		return $items;
	}
	if ( of_get_option( 'tokopress_minicart_header' ) && $args->theme_location == 'header_menu' ) {
		$wc_cart_url = function_exists('wc_get_cart_url') ? wc_get_cart_url() : WC()->cart->get_cart_url();
		$items .= '<li class="menu-item menu-item-minicart"><a href="'.esc_url($wc_cart_url).'"><i class="fa fa-shopping-cart"></i><span class="minicart">'.WC()->cart->get_cart_subtotal().'</span></a></li>';
	}
	return $items;
}
